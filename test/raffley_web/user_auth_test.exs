defmodule RaffleyWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RaffleyWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/users/token/create"
    ...
    test "redirects to settings when user is already logged in", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == "/users/token/create"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RaffleyWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RaffleyWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
